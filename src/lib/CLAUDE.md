...
- `upload-queue.ts` — FIFO single consumer, head-of-line retry; error policy keyed on the server `code`; rows deleted only after a 2xx
...
- `engine.ts` — `RecorderEngine`: orchestrates everything above outside React. Commands are phase-guarded; seq/part/seal bookkeeping is synchronous in `onChunk` (an async assembler would mis-stamp chunks); segment opens are eager; rotation starts the new run before stopping the old; unrequested recorder stops mean "mic lost", never finalize. `bootstrap()` is memoized and never takes over. Disposed only on terminal outcomes/eviction, never by a React unmount.
