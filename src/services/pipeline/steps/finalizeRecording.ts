...
export async function runFinalizeRecordingStep(
  ...
  if (recording.finalizedUploadId) {
    return finishBookkeeping(sessionId, recording, recording.finalizedUploadId);
  }
  ...
  try {
    ...
    // 2. Join segments. Same codec/params by construction, so this is a
    //    stream copy (concat demuxer), not a re-encode.
    assertActive(ctx);
    let assembledPath: string;
    if (segmentFiles.length === 1) {
      assembledPath = segmentFiles[0];
    } else {
      ...
      assembledPath = path.join(workDir, `assembled${ext}`);
    ...
    const duration = await probeAudioDurationSeconds(assembledPath);
    ...
    const result = await finishBookkeeping(sessionId, recording, upload.id);
